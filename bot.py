...
async def seconds_until_next_utc_run(hour_utc=24, minute_utc=30):
...
@tasks.loop(hours=24)
async def daily_job():
    """Automatycznie wysyła screena codziennie o 01:30 czasu polskiego."""
    if not CHANNEL_ID:
...
async def setup_hook():
    ...
    async def starter():
        wait = await seconds_until_next_utc_run(24, 30)
        print(f"⏳ Pierwsze uruchomienie za {int(wait)}s (-> 24:30 UTC / 01:30 czasu polskiego)")
